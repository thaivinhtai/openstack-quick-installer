...
EOF
    ...
    if [ "$1" == "controller" ] || [ "$1" == "all-in-one" ]; then
        ...
        openstack endpoint create --region ${REGION_NAME} \
            compute internal http://$INTER_FQDN_CTL:8774/v2.1
        openstack endpoint create --region ${REGION_NAME} \
    ...
    if [ "$1" == "controller" ]; then
        ...
        print_install "Install NOVA"
        apt-get -y install nova-api nova-conductor nova-novncproxy nova-scheduler nova-compute
        backup_config $nova_conf
        backup_config $novacom_conf
